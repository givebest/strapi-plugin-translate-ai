...
class AiTranslator {
  constructor(options) {
    const { apiKey, apiUrl, apiModel, maxTokens, maxCompletionTokens, temperature, generalPromt } =
      options
    ...
    this.maxCompletionTokens = maxCompletionTokens || 128000
  ...
  async translate(options) {
    ...
    try {
      ...
      if (this.apiModel.toLowerCase() === 'gpt-5') {
        delete bodyData.max_tokens
        bodyData.max_completion_tokens = this.maxCompletionTokens
      }
